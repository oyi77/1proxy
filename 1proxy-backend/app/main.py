...
async def seed_and_scrape():
    """
    Emergency recovery: create admin, seed sources, and scrape.
    Use when sources/proxies are empty. No auth required.
    """
    async with AsyncSessionLocal() as session:
        ...
        for source in sources:
            try:
                ...
                for p in proxies:
                    ...
                    ip = p.ip if hasattr(p, "ip") else p.get("ip")
                    if url and ip:
                        await db_storage.add_proxy(session, {"url": url, "ip": ip})
                        total_added += 1
                        total_scraped += 1
            except Exception as e:
        ...
        return {
            "status": "ok",
            "sources_seeded": seed_count,
            "proxies_scraped": total_scraped,
            "proxies_added": total_added,
        }
