...
async def background_scraper_worker(interval_minutes: int = 10):
    ...
    logger.info(f"🔄 Background scraper started (runs every {interval_minutes} minutes)")
...
async def scrape_all_sources():
    ...
    logger.info("🔄 Starting automatic proxy scraping...")
        ...
        async with AsyncSessionLocal() as session:
            ...
            for source_db in sources_db:
                try:
                    ...
                    source = SourceConfig(
                        url=source_db.url,
                        type=SourceType(source_db.type)
                    )
                    ...
                    logger.info(f"✅ Scraped {len(proxies)} proxies from {source_db.name} (added {added} new)")
            ...
            logger.info(f"✅ Auto-scraping complete: {total_scraped} scraped, {total_added} new proxies added")
