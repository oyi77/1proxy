...
from ._validator_validationresult import (
    ValidationResult,
)
...
class OptimizedProxyValidator:
    ...
    def __init__(
        ...
        if config is None:
            config = _facade.get_default_config()
            config.max_concurrent_validations = max_concurrent
            config.connectivity_timeout = timeout
        self.config = config
        self.session: _facade.Optional[_facade.aiohttp.ClientSession] = None
        self.connector: _facade.Optional[_facade.aiohttp.TCPConnector] = None
        self.semaphore: _facade.Optional[_facade.asyncio.Semaphore] = None
        
        # Caches for external API responses
        self._geo_cache = _facade.LRUCache(max_size=5000, default_ttl=self.config.geo_cache_ttl)
        self._proxy_type_cache = _facade.LRUCache(max_size=5000, default_ttl=self.config.proxy_type_cache_ttl)
        self._validation_cache = _facade.LRUCache(max_size=10000, default_ttl=300)  # 5 min for validation results
        
        # Response time tracking for p95
        self._response_times: _facade.Dict[str, _facade.List[int]] = {}
        self._response_times_lock = _facade.asyncio.Lock()
    ...
    async def _ensure_session(self) -> None:
        ...
        if self.session is None:
            if self.connector is None:
                self.connector = _facade.aiohttp.TCPConnector(
            ...
            if self.semaphore is None:
                self.semaphore = _facade.asyncio.Semaphore(self.config.max_concurrent_validations)
            
            # Default timeout for all requests
            timeout = _facade.aiohttp.ClientTimeout(
            ...
            self.session = _facade.aiohttp.ClientSession(
    ...
    async def validate_connectivity_fast(self, proxy_url: str) -> Tuple[bool, Optional[int], Optional[str]]:
        ...
        try:
            start = _facade.time.perf_counter()
            async with self.semaphore:
                async with self.session.get(
                    ...
                    latency_ms = int((_facade.time.perf_counter() - start) * 1000)
        ...
        except _facade.aiohttp.ClientProxyConnectionError:
            return False, None, "Proxy connection failed"
        except _facade.aiohttp.ClientConnectorError as e:
        ...
        try:
            async with self.semaphore:
                async with self.session.get(
                    ...
                    proxy=proxy_url,
                    ssl=False,
                    timeout=_facade.aiohttp.ClientTimeout(total=5.0)
                ) as resp:
                    if resp.status != 200:
                        return None
        ...
        try:
            async with self.semaphore:
                async with self.session.get(
                    ...
                    timeout=_facade.aiohttp.ClientTimeout(total=self.config.external_api_timeout)
                ) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                        result = {
            ...
            async with self.semaphore:
                async with self.session.get(
                    ...
                    if resp.status == 200:
                        ...
                        result: _facade.Dict[str, _facade.Optional[object]] = {
    ...
    async def check_ssl_validity_fast(self, proxy_url: str) -> Optional[bool]:
        """Fast SSL check"""
        try:
            ssl_context = _facade.ssl.create_default_context(cafile=_facade.certifi.where())
            async with self.semaphore:
                async with self.session.get(
                    "https://www.google.com",
                    proxy=proxy_url,
        ...
        try:
            async with self.semaphore:
                async with self.session.get(
                    ...
                    proxy=proxy_url,
                    ssl=False,
                    timeout=_facade.aiohttp.ClientTimeout(total=3.0)
                ) as resp:
                    if resp.status == 200:
    ...
    async def validate_phase1(self, proxy_url: str, ip: str) -> ValidationResult:
        ...
        """
        cache_key = _facade.hashlib.md5(f"{proxy_url}:{ip}:phase1".encode()).hexdigest()
        ...
        if not is_valid:
            ...
            result = _facade.ValidationResult(success=False, error_message=error)
        ...
        if latency_ms and latency_ms > self.config.fast_fail_latency_ms:
            self._stats["validations_failed"] += 1
            result = _facade.ValidationResult(
                success=False, 
    ...
    async def validate_phase2(
        ...
        """
        cache_key = _facade.hashlib.md5(f"{proxy_url}:{ip}:phase2".encode()).hexdigest()
        cached = await self._validation_cache.get(cache_key)
        if cached:
            self._stats["cache_hits"] += 1
            return cached
        ...
        if proxy_type == "unknown":
            old_geo, old_type = await _facade.asyncio.gather(
    ...
    async def validate_batch_optimized(
        ...
        for i in range(0, len(proxies), batch_size):
            ...
            phase1_results = await _facade.asyncio.gather(*phase1_tasks, return_exceptions=True)
            ...
            for idx, (phase1_result, (proxy_url, ip)) in enumerate(zip(phase1_results, batch)):
                if isinstance(phase1_result, Exception):
                    all_results.append((
                        proxy_url,
                        _facade.ValidationResult(success=False, error_message=str(phase1_result)[:100])
                    ))
            ...
            if phase2_tasks:
                phase2_results = await _facade.asyncio.gather(*phase2_tasks, return_exceptions=True)
                ...
                for idx, phase2_result in zip(phase2_indices, phase2_results):
                    ...
                    if isinstance(phase2_result, Exception):
                        all_results.append((
                            proxy_url,
                            _facade.ValidationResult(success=False, error_message=str(phase2_result)[:100])
                        ))
                    else:
